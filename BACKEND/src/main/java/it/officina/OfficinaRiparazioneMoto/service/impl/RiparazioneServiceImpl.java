...
/**
 * Implementation of the {@link RiparazioneService} interface.
 * <p>
 * This service provides operations for managing repairs, including retrieval of
 * repair lists,
 * saving new repairs, updating repair status, and deleting or rejecting
 * repairs.
 * The service interacts with data access objects (DAOs), mappers, and other
 * services to
...
public class RiparazioneServiceImpl implements RiparazioneService {
    ...
    @Override
    public RiparazioneDto salvaRiparazioneAccettata(RiparazioneDto riparazione) throws BadRequestException {
    ...
    /**
     * Generates a unique service code for a repair.
     * <p>
     * The code is generated by creating a random alphanumeric string of a fixed
     * length,
     * then inserting the last three digits of the current timestamp at random
     * positions.
     * </p>
     *
     * @return a unique service code as a {@link String}
     */
    private String generaCodiceServizio() {
        ...
        StringBuilder codice = new StringBuilder(
                RandomStringUtils.random(LUNGHEZZA_CODICE, 0, 0, false, false, CARATTERI.toCharArray(), random));
        ...
        for (char carattere : timestampShort.toCharArray()) {
            int posizioneCasuale = random.nextInt(codice.length() + 1); // +1 permette di metterlo anche nell'ultimo
    ...
    public List<RiparazioneMotoClienteDto> getListaRiparazioniMotoClienteDto(UUID idUtenteReg, UUID idUtenteMec,
            int... stati) throws IllegalArgumentException {
    ...
    @Override
    public void aggiornaStatoRiparazione(UUID idRiparazione, UtenteDto utenteMec)
    ...
    @Override
    public RiparazioneDto getRiparazioneDto(UUID idRiparazione) throws BadRequestException {
        return mapper.toDto(riparazioneDao.findById(idRiparazione)
                .orElseThrow(() -> new BadRequestException(ErrorManager.RIPARAZIONE_NON_TROVATA)));
    }

    @Override
    public void eliminaRiparazione(UUID idRiparazione) throws BadRequestException {
    ...
    @Override
    public void rifiutaRiparazione(UUID idRiparazione) throws BadRequestException {
        Riparazione riparazione = riparazioneDao.findById(idRiparazione)
                .orElseThrow(() -> new BadRequestException(ErrorManager.RIPARAZIONE_NON_TROVATA));
