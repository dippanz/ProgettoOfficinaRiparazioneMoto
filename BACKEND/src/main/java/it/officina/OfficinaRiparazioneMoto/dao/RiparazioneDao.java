...
public interface RiparazioneDao extends JpaRepository<Riparazione, UUID> {
        ...
        /**
         * Retrieves a boolean for specified if in repairs there are a moto not
         * completed or not refused
         * 
         * @param targa the license plate of the vehicle
         * @return true if there are a moto not completed, false otherwise
         */
        @Query("SELECT COUNT(r) > 0 FROM Riparazione r JOIN r.moto m WHERE m.targa = :targa AND r.stato.id != 5 AND r.stato.id != 2")
        boolean existsByMotoTargaNotCompleted(@Param("targa") String targa);
}
